  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        <h2 className="line-clamp-1 font-medium text-neutral-900">{plant.name}</h2>
        {plant.scientific_name && (
          <p className="line-clamp-1 text-xs italic text-neutral-500">{plant.scientific_name}</p>
        )}
        <p className="mt-auto text-xs text-neutral-500">Comprada em {purchaseDate}</p>
      </div>
    </Link>
  );
}
